...
| **ledger** | `decimal.Decimal` for amounts. Booking methods: `STRICT` (default), `NONE`, `FIFO`, `LIFO`, `HIFO`, `AVERAGE`; `Inventory.Book` decides augment vs. reduce (beancount's `is_reduced_by`). |
