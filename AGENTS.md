...
| **ledger** | `decimal.Decimal` for amounts. Booking methods: `STRICT` (default), `NONE`, `FIFO`, `LIFO`, `HIFO`, `AVERAGE` (like v2, every reduction under it fails, and a merge cost `{*}` is reported and booked like `{}`); `Inventory.Book` decides augment vs. reduce (beancount's `is_reduced_by`). Apply leaves booked postings in `txn.Postings`, in beancount's booked order (grouped by the currency each posting balances in; an amount-less posting becomes one posting per residual currency); `BodyItems` keeps the source layout for the formatter. |
