...
| **internal/pydecimal** | Python `decimal` behaviour beancount depends on and shopspring lacks (arithmetic rounded to 28 significant digits, division with exact-quotient exponents, `normalize`). Add, subtract, multiply and divide only with `Add`, `Sub`, `Mul` and `Quo`; `forbidigo` rejects shopspring's `Add`/`Sub`/`Mul`/`Div`/`DivRound` elsewhere. Use it wherever a number must keep beancount's precision. |
