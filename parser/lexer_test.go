...
func TestLexerBasicTokens(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, len(tt.want), len(tokens))

			for i, tok := range tokens {
				assert.Equal(t, tt.want[i], tok.Type, "token %d type mismatch", i)
			}
		})
	}
}
...
func TestLexerNegativeNumbers(t *testing.T) {
	// After adding expression support, negative numbers are tokenized as MINUS + NUMBER
	// This allows proper handling of both unary minus (e.g., -5) and binary subtraction (e.g., 5 - 3)
	tests := []struct {
		input      string
		wantTokens []TokenType
		wantValues []string
	}{
		{"-123", []TokenType{MINUS, NUMBER, EOF}, []string{"-", "123"}},
		{"-123.45", []TokenType{MINUS, NUMBER, EOF}, []string{"-", "123.45"}},
	}
	...
	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			...
			assert.Equal(t, len(tt.wantTokens), len(tokens))

			for i, wantType := range tt.wantTokens {
				assert.Equal(t, wantType, tokens[i].Type, "token %d type mismatch", i)
				if i < len(tt.wantValues) {
					got := tokens[i].String(lexer.source)
					assert.Equal(t, tt.wantValues[i], got, "token %d value mismatch", i)
				}
			}
		})
	}
...
`
	...
	for i, tok := range tokens {
		assert.Equal(t, expectedTypes[i], tok.Type, "token %d type mismatch (text: %q)", i, tok.String(lexer.source))
	}
}
...
`
	...
	for i := 0; i < 3; i++ {
		assert.Equal(t, ACCOUNT, tokens[i].Type, "token %d: expected ACCOUNT", i)
	}
